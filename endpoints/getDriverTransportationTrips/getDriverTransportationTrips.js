...
const getDriverTransportationTrips = async (cognitoSub, isAll) => {
  if (!cognitoSub) {
  ...
  try {
    ...
    const query = knexInstance('transportation_trip as trip')
      .join(
      ...
      .groupBy(
        ...
        'vehicle_type.vehicle_type_id',
        'transportation_trip_status.transportation_trip_status_id',
        'po_request.created_at'
      )
      .orderBy('po_request.created_at', 'asc')
    ...
    if (!isAll) {
      ...
      query.andWhere('trip.created_at', '>', twentyFourHoursAgo.toISOString());
    }
